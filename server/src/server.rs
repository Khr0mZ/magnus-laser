...
impl RoomManager {
    pub fn register_seed(&mut self, seed: RoomSeed) {
    ...
    pub fn remove_peer(&mut self, code: &str, peer_id: &str) -> Option<RemovalOutcome> {
    ...
    fn cleanup_expired(&mut self) -> Vec<RemovalOutcome> {
        ...
        for (code, room) in &self.rooms {
            if room.last_activity.elapsed() > ROOM_TTL {
    ...
    pub fn broadcast_host_leaving(&self, code: &str) {
...
pub enum RemovalOutcome {
    PeerLeft {
        peers: Vec<PeerInfo>,
        senders: Vec<mpsc::UnboundedSender<OutboundMsg>>,
    },
    RoomClosed {
        senders: Vec<mpsc::UnboundedSender<OutboundMsg>>,
    },
...
pub fn create_router_with_state(state: AppState) -> Router {
    // Start the cleanup task for expired rooms
    spawn_cleanup_task(state.manager.clone());
