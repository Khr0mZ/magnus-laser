        ...
        <Container maxWidth={false} sx={{ pt: 0.5 }}>
            ...
            <Grid container spacing={3} sx={{ position: 'relative', zIndex: 3 }}>
                ...
                {Object.values(ModuleTypes)
                    .filter((module) => module !== ModuleTypes.BUILDING)
                    .filter((module) => module !== ModuleTypes.CHARACTER)
                    .filter((module) => module !== ModuleTypes.ITEM)
                    .filter((module) => module !== ModuleTypes.BOUNTY)
                    .filter((module) => module !== ModuleTypes.GANG)
                    .filter((module) => module !== ModuleTypes.FIXER_JOB)
                    .filter((module) => module !== ModuleTypes.CHARACTER_CREATOR)
                    .map((module) => (
                        <Grid size={{ xs: 12, md: 6, lg: 4, xl: 3, xxl: 3 }} key={module}>
                            <Card
                                onClick={() => {
                                    navigate(NavigationPaths[module as keyof typeof NavigationPaths])
                                }}
                                sx={{
                                    position: 'relative',
                                    borderRadius: '4px',
