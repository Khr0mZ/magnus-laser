                        ...
                        >
                            {columns.map((column) => (
                                ...
                                >
                                    ...
                                    {moduleType === ModuleTypes.BOUNTY && t(`bounties.labels.${column.label}`)}
                                </TableCell>
                            ))}
                            ...
                            .map((target, index) => {
                                ...
                                switch (moduleType) {
                                    ...
                                    case ModuleTypes.ITEM:
                                        ...
                                        break
                                    case ModuleTypes.CHARACTER:
                                        name = (target as Character).name
                                        color = colors.neons.cyan.default
                                        break
                                    case ModuleTypes.BOUNTY:
                                        name = (target as Bounty).character.name
                                        color = colors.neons.cyan.default
                                        break
                                }
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                {moduleType === ModuleTypes.BOUNTY
                                                    ? typeof (target as Bounty).character === 'string'
                                                        ? resolveCharacterReference(
                                                              (target as Bounty).character as unknown as string
                                                          )?.name || '—'
                                                        : (target as Bounty).character.name
                                                    : name}
                                            </Typography>
                                        </TableCell>
                                        ...
                                        {columns.slice(2).map((column) => {
                                            ...
                                            if (moduleType === ModuleTypes.GANG) {
                                                ...
                                                if (column.key === 'name') {
                                                    displayValue = (target as FixerJob).name
                                                } else if (
                                                    ...
                                                    displayValue = t(
                                                        `characters.type.${
                                                            typeof (target as Bounty).character === 'string'
                                                                ...
                                                                : (target as Bounty).character.type
                                                        }`
                                                    )
                                                } else if (column.key === 'speciality') {
                                                    displayValue = t(
                                                        `bounties.speciality.${(target as Bounty).speciality}`
                                                    )
                                                } else if (column.key === 'rep') {
                                                    displayValue = t(`bounties.rep.${(target as Bounty).rep}`)
                                                } else if (column.key === 'crimes') {
                                                    displayValue = String((target as Bounty).crimes.length)
                                                } else if (column.key === 'totalBounty') {
                                                    displayValue =
                                                        Intl.NumberFormat('en-US').format(
                                                            (target as Bounty).crimes.reduce(
                                                                ...
                                                                baseBountyRewardPerSpeciality[
                                                                    (target as Bounty).speciality
                                                                ]
                                                        ) + ' €$'
                                                } else {
                                                    displayValue = processBountyValueForDisplay(
                                                        column.key,
                                                        (target as Bounty)[column.key as keyof Bounty],
                                                        t
                                                    )
                                                }
                                            }
